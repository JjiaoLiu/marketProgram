...
class JobCardActionMerchant extends Component {
    config = {
    ...
    handleDelete(e) {
        e.stopPropagation();
        request({
            url: `/merch/jobs?id=${this.props.job.id}`, method: 'DELETE'
        }).then(() => {
            this.hideDel(e);
            this.props.onUpdate();
        })
    ...
    render() {
        ...
        return (
            <View className='job-card'>
                <View className='detail-1' style={{borderBottom: '1px solid #f3f3f3'}}
                      onClick={this.toCompanyPage.bind(this, job.merchId)}>
                    <Text className='detail-11'>{job.merchantName}</Text>
                    <Image src={IconRightArr} className='icon_back'/>
                </View>
                <View onClick={this.toJobDetaillPage.bind(this)}>
...
export default JobCardActionMerchant;